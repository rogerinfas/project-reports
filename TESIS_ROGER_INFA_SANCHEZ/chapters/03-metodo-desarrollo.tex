...
\begin{enumerate}
    ...
    \end{verbatim}
    De esta forma, solo la intersección de ambas condiciones (el caso de decisión verdadero/verdadero) desencadena la inicialización del simulador, el cálculo de progreso e interpolación del punto espacial vía PostGIS, y su subsecuente difusión hacia los clientes de mapa a través de WebSockets (\texttt{this.trackingGateway.server.to(routeId).emit('vehicle\_update', ...)}).
\end{enumerate}
