...
\begin{enumerate}
    \item \textbf{Partición de Equivalencias:} Aplicado a los estados del vehículo (\texttt{ACTIVE}, \texttt{INACTIVE}, \texttt{MAINTENANCE}) y a la geolocalización, verificando que los registros GPS se encuentren exclusivamente dentro del bounding box geoespacial de la provincia de Arequipa (Latitud \texttt{[-16.6, -16.2]} y Longitud \texttt{[-71.7, -71.2]}).
    \item \textbf{Análisis de Valores Límite:} Diseñado para variables operativas como la velocidad (\texttt{speedKph}), evaluando las fronteras entre el estado estacionario (\texttt{0.0} km/h), los límites permitidos de circulación urbana (\texttt{100.0} km/h) y valores negativos o excesivos mediante esquemas de validación de esquemas (DTOs). Asimismo, se evalúan los límites del identificador de placa vehicular de 6 a 7 caracteres válidos según la regulación de transporte.
    \item \textbf{Tabla de Decisiones:} Implementado para el flujo de sincronización de tracking, asegurando que únicamente los buses con asignación vigente a una ruta y con estado operativo \texttt{ACTIVE} actualicen su posición en tiempo real e inicien el cálculo automático de arribo (ETA) a los paraderos.
\end{enumerate}
...
La validación de la infraestructura en producción (VPS Linux) se orientó a la estabilidad del canal bidireccional de comunicación en tiempo real (\textit{WebSockets} mediante \textit{Socket.io}). Debido al comportamiento de \textit{handshake} inicial de Socket.io mediante peticiones HTTP polivalentes, se configuraron y probaron con éxito las etiquetas de \textbf{Sticky Sessions} (sesiones pegajosas) mediante cookies a nivel del proxy inverso \textbf{Traefik}, redirigiendo de manera segura todo el tráfico del websocket (\texttt{/socket.io}) al mismo contenedor de backend.

Las pruebas de comunicación en red confirmaron que al forzar el transporte directo a protocolo \texttt{['websocket']} puro en el cliente Next.js, se reducen las fallas de conexión inicial y se asegura el establecimiento rápido del túnel seguro (HTTPS/WSS) firmado mediante certificados TLS automáticos de \textbf{Let's Encrypt}. Asimismo, el flujo de integración continua (\textbf{GitHub Actions}) valida automáticamente la calidad del código, sintaxis y migración de Prisma antes de orquestar el despliegue automático del contenedor de producción.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{assets/captura-devops-despliegue.png}
    \figcaptionwithauthor{Flujo de integración continua en GitHub Actions y orquestación de despliegue con Docker y Traefik}
\end{figure}
...
\begin{itemize}
    \item \textbf{Responsividad y Usabilidad:} La interfaz web basada en componentes reactivos de React 19 y Tailwind CSS 4 responde fluidamente a dispositivos móviles y escritorios, asegurando que el mapa interactivo ocupe el área correcta y los menús de filtrado de rutas sean totalmente accesibles.
