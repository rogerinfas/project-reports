...
(async () => {
  ...
  if (firstProd) {
      ...
      // 11. PESTAÑA: Configuración Precios (CRM)
      const tabCRM = await page.$('a[role="tab"]:has-text("Configuración Precios (CRM)")');
      if (tabCRM) {
      ...
      // 12. PESTAÑA: Datos del Tour
      const tabTour = await page.$('a[role="tab"]:has-text("Datos del Tour")');
      if (tabTour) {
      ...
      // 13. PESTAÑA: Contenidos Multi-Idioma
      const tabIdioma = await page.$('a[role="tab"]:has-text("Contenidos Multi-Idioma")');
      if (tabIdioma) {
      ...
      // EXTRA: Dump the DOM so the assistant can see what fields are inside to highlight them if needed.
      const domContent = await page.content();
      const fs = require('fs');
      fs.writeFileSync('dom_producto_custom.html', domContent);
      console.log('DOM guardado para analisis');
  }
